...
namespace ococo {
    ...
    constexpr T right_full_mask(){
        ...
        return (size==0) ? 0 : (((static_cast<T>(0x1)<<(size-1)) - 1 ) << 1 ) | 1;
    }
    ...
    inline T get_right_bits(T pattern){
        static_assert(rightmost_bit+size<=8*sizeof(T),"Exceeding data type borders.");
    ...
    inline T get_left_bits(T pattern){
        return get_right_bits<T,size,8*sizeof(T)-leftmost_bit-size>(pattern);
    }
    ...
    inline T set_left_bits(T pattern){
        return set_right_bits<T,8*sizeof(T)-leftmost_bit-size,size>(pattern);
    }
#endif
