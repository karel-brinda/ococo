        ...
        try {
            ...
            if (vm.count("counters")) {
                if (counters_str.compare("ococo16") == 0) {
                    counter_configuration = OCOCO16;
                    counters_str_descr="ococo16 (16 bits per position, 3bits per nucleotide counter)";
                } else if (counters_str.compare("ococo32") == 0) {
                    counter_configuration = OCOCO32;
                    counters_str_descr="ococo32 (32 bits per position, 7bits per nucleotide counter)";
                } else if (counters_str.compare("ococo64") == 0) {
                    counter_configuration = OCOCO64;
                    counters_str_descr="ococo64 (64 bits per position, 15bits per nucleotide counter)";
                } else {
                    ococo::error("Unknown counter configuration '%s'. Possible modes are "
                                 "'ococo16', 'ococo32', and 'ococo64'.\n",
                                 counters_str.c_str());
                    correctly_initialized=false;
                    return_code=-1;
                    return;
